...
class ToolExecutor:
    ...
    # -----------------------------------------------------------------------
    # v0.3.0/TASK-015 -- Wails-call helper for mac_* / spotify_* branches.
    #
    # Sends a ``tool_call`` WS message with the CamelCase Wails method name
    # (e.g. ``MacOpenApp``) and awaits the Go app's ``tool_result``,
    # normalising the response into a uniform ``{"result", "error"}``
    # envelope. Mirrors the helper TASK-010 also wants -- if a parallel
    # agent adds an identical method first the merge resolves cleanly
    ...
    async def _call_wails(
        ...
        *,
        ...
        """Invoke a Wails-bound App method by CamelCase name.

        Args:
            method: Wails method name (e.g. ``MacOpenApp``).
            args: Positional args list, or kwargs dict. Defaults to ``[]``.
            timeout: Max seconds to wait for the result.

        Returns:
            ``{"result": <value>, "error": <str | None>}``.  Never raises:
            transport / timeout / protocol failures are all surfaced via
            the ``error`` key so the calling branch can shape a friendly
            spoken message.
        """
        if args is None:
            args = []
    ...
    async def execute(
        ...
        *,
        ...
        # -------------------------------------------------------------------
        # v0.3.0/TASK-015 -- mac_* executor branches.
        #
        # Each branch validates required params in Python (so a missing
        # arg surfaces as a friendly spoken message rather than a Go-side
        # exec error), dispatches to the matching Wails-bound App method
        # via ``_call_wails``, and shapes the result into the
        # ``{"ok": bool, "message": str}`` envelope the daemon LLM layer
        # expects.
        #
        # The policy-deny path (macctl.ErrPolicyDeny on the Go side) is
        # detected by substring match on the error string -- Go errors
        # propagate as plain strings over the WS bridge, so we can't use
        # errors.Is across the language boundary, but the canonical
        # marker "ErrPolicyDeny" / "denied by policy" is stable enough
        # to key on. Falling back to a generic failure message if the
        # marker is absent keeps the branch resilient to future
