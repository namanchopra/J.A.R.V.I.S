...
// capture_darwin.go — PortAudio-backed microphone capture for macOS.
//
// This is the only file in the package (and the repo) that imports
// github.com/gordonklaus/portaudio, whose cgo preamble resolves the system
// portaudio via `pkg-config portaudio-2.0`. The dylib is bundled in the .app
// (Contents/Frameworks/) and present on dev machines via Homebrew. Windows
// builds use the stub in capture_other.go instead — see the package doc.
