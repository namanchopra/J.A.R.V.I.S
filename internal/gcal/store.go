// Google Calendar credential storage — sibling to internal/spotify/store.go.
//
// Why a sibling file (~/.jarvis/gcal.json) rather than nesting under
...
// struct field. Adding `GCal model.GCalConfig` to config.Config would
// cause convertValues to appear on the generated Config class, which
// breaks every Settings panel that constructs a config via spread
...
const configFilename = "gcal.json"

// ConfigPath returns the canonical path Jarvis reads and writes Google
// Calendar credentials at — ~/.jarvis/gcal.json. Routed through
// internal/paths so HOME redirection in tests is honored.
func ConfigPath() string {
	return filepath.Join(paths.JarvisHome(), configFilename)
}

// LoadConfig reads and parses the persisted GCalConfig at path. Returns
...
func LoadConfig(path string) (model.GCalConfig, error) {
	if path == "" {
		return model.GCalConfig{}, errors.New("LoadConfig: path is required")
	}
	data, err := os.ReadFile(path)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return model.GCalConfig{}, nil
		}
		return model.GCalConfig{}, fmt.Errorf("LoadConfig: read %s: %w", path, err)
	}
	var cfg model.GCalConfig
	if err := json.Unmarshal(data, &cfg); err != nil {
		return model.GCalConfig{}, fmt.Errorf("LoadConfig: parse %s: %w", path, err)
	}
	return cfg, nil
}
...
func SaveConfig(path string, cfg model.GCalConfig) error {
	if path == "" {
