...
<!-- DATE PLACEHOLDER: replace with real release date during TASK-031 -->
## [0.1.0] - 2026-05-11
...
- Comprehensive Settings UI rebuilt around 5 tabs: Connections, Voice, Behavior, Diagnostics, and Advanced
...
- TTS provider dropdown (VibeVoice, Kokoro, Edge, Cartesia) and STT model dropdown (Whisper small.en, Whisper tiny.en, faster-whisper)
...
- DMG build pipeline scripts: `fetch-python.sh` (pinned python-build-standalone CPython 3.13.13, SHA256-verified), `build-daemon-venv.sh` (relocatable venv with `@VENV_ROOT@` placeholder), `fetch-models.sh` (VibeVoice-Realtime-0.5B + voice preset + mlx-community Whisper-small.en, idempotent), and a `post-build.sh` packaging step
